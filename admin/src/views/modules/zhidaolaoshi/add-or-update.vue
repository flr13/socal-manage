	...
	<div class="addEdit-block" :style='{"padding":"30px","background":""}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="管理社团" prop="guanlishetuan">
					<el-input v-model="ruleForm.guanlishetuan" placeholder="管理社团" clearable  :readonly="ro.guanlishetuan"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="管理社团" prop="guanlishetuan">
					<el-input v-model="ruleForm.guanlishetuan" placeholder="管理社团" readonly></el-input>
				</el-form-item>
			</template>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='guanlishetuan'){
							this.ruleForm.guanlishetuan = obj[o];
							this.ro.guanlishetuan = true;
							continue;
						}
				}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhidaolaoshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhidaolaoshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
